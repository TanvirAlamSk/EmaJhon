...
const OrderReviewCart = ({ ordersData, handleClearCart }) => {
    let quantity = 0
    let price = 0;
    let shippingPrice = 0;
    ...
    return (
        <div className='sticky'>
            ...
            <Link to="/review-order"><button className='bg-yellow-500 w-full py-3 rounded-md text-white'>Order Process<ArrowRightIcon className='ml-2 h-5 w-6 inline font-bold'></ArrowRightIcon></button></Link>
        </div>
    );
};
